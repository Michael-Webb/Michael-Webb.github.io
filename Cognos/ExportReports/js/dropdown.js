define(() => {
  ...
  class DropdownControl {
    ...
    initialize(oControlHost, fnDoneInitializing) {
      ...
      this.GlassContext = Application && Application.GlassContext ? Application.GlassContext : null;
    ...
    draw(oControlHost) {
      ...
      c.appendChild(title);
      this._selectElementId = this.oControlHost.generateUniqueID();
    ...
    showExampleDialog() {
      ...
      // --- Use the ID stored during draw ---
      const selectId = this._selectElementId; // Store the ID
      if (!selectId) {
        console.error(
          "DropdownControl: Cannot show dialog, unique ID (_selectElementId) was not generated or stored during draw."
        );
        alert("Internal Error: Dialog ID missing.");
        return; // Don't proceed if the ID is missing
      }
      console.log("DropdownControl: Using stored select ID:", selectId);
      ...
      const dialogConfig = {
        ...
        callback: {
          ...
          general: (response) => {
            ...
            const selectElement = document.getElementById(selectId); // Use stored ID
