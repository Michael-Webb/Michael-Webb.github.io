      ...
      if (this._promptName) {
        ...
        if (promptControl) {
          ...
          if (currentValues && currentValues.length > 0) {
            if (currentValues[0].start) {
              //Handle Range Parameter
              this._selectedItems = currentValues.map((range) => ({
                use: range.start.use,
                display: range.start.display,
              }));
            } else {
              //Handle Normal Parameter
              this._selectedItems = currentValues.map((item) => ({
                use: item.use,
          ...
          console.warn("MyDataLoggingControl - Could not find prompt control for parameter: ", this._parameterName);
        }
      }
