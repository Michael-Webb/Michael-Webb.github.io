define(() => {
    ...
    class CustomControl {
      ...
      initialize(oControlHost, fnDoneInitializing) {
        ...
        this.caseInsensitiveDefault =
          caseInsensitiveDefault !== undefined ? Boolean(caseInsensitiveDefault) : true;
       ...
       */
      generateContentHTML({ valueUseCol, valueDispCol, groupingParamName, groupVals, groupingValUseCol, groupingValDispCol }) {
        // Use the default check state for the "case-insensitive" checkbox.
        const caseInsensitiveChecked = this.caseInsensitiveDefault ? "checked" : "";
        let html = `
          <div class="header">
            <div class="search-container">
              <div class="search-wrapper">
                <input type="text" class="search-input" id="${this.searchId}" placeholder="Search options..." aria-label="Search options" />
                <button type="button" class="search-icon" id="searchIcon" aria-label="Search">
                  ${this.getSearchIconSVG(true)}
                </button>
              </div>
              <button class="advanced-search" aria-expanded="false" aria-controls="${this.searchControlsId}" id="${this.advancedBtnId}">
                <span>Advanced</span>
        ...
        // If a data store exists, render the options.
      ...
      applyEventListeners() {
        ...
        this.elements.search.addEventListener("input", debounce(() => {
      ...
      getParameters() {
        ...
        const selectedValues = Array.from(
          this.elements.dropdown.querySelectorAll('input[type="checkbox"]:checked')
        ).map(cb => cb.value);
        const params = [{
      ...
      toggleAllItems(checked) {
        const visibleCheckboxes = Array.from(
          this.elements.dropdown.querySelectorAll('input[type="checkbox"]')
        ).filter(cb => {
          const checkboxItem = cb.closest(".checkbox-item");
