        ...
        if (!isMultiple) {
          ...
          if (this.hasGrouping) {
            ...
            //Make sure to select item values outside of the group parameter
             this.m_itemCheckboxes.forEach(itemCb => {
              if (!this.mainParamValues.includes(itemCb.value) && !itemCb.checked && !this.groupParamValues.includes(itemCb.getAttribute('data-group'))) {
                 itemCb.checked = false;
              }
            });
          } else {
