define([], function () {
  ...
  DocumentsOnline.prototype.initialize = function (oControlHost, fnDoneInitializing) {
    ...
    function remove_image(img_id) {
      ...
      return document.getElementById(img_id).setAttribute("data-status", "no document found");
    }
    function fetchDataFromSpanElements(
      ...
      const spanElements = document.querySelectorAll("span[name='attachments']");
      const requests = [...spanElements].map((span) => {
        ...
        if ((pExclude_tog && sReference.startsWith(pExclude_val)) === true) {
          ...
          if (get_status === "new") {
            ...
            const fetchUrl =
              baseUrl + "arg=" + encodeURI(fEncode(url_Decode(sArg).replaceAll("\\", ""))) + "&env=" + url_Decode(sEnv);
            ...
            return console.log(sReference, " no document found");
          } else if (status === "document found" || status === "document loading") {
