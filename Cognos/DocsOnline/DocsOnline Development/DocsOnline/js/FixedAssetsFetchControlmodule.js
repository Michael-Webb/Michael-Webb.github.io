// documentAttachmentsModule.js

/**
 * Initializes the document attachments module.
 * It performs the following steps:
 * 1. Extracts sessionID, token, appBaseURL, and jobBaseURL from the first span with id starting with "documentOnline-"
 * 2. Performs sequential fetches for the FAUPAS screen, API token, and token validation.
 * 3. Processes all spans (id starting with "documentOnline-") to fetch attachments and create a button (with a paperclip SVG) that opens a modal.
 */
export async function initDocumentAttachments() {
  ...
  const paperclipSVG = `<svg xmlns="http://www.w3.org/2000/svg" width="${ICON_WIDTH}" height="${ICON_HEIGHT}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-clock"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>`;

  // Helper function to create and show the modal with document links
