(async function () {
    ...
    async function fetchFaupasScreen(appBaseURL, jobBaseURL) {
      try {
    ...
    async function fetchApiToken(jobBaseURL, sessionID, token) {
      try {
    ...
    async function validateSecurityToken(jobBaseURL, appBaseURL, sessionID, token, apiToken) {
      try {
    ...
    async function authenticate(appBaseURL, jobBaseURL, sessionID, token) {
      try {
        // Step 1: Fetch FAUPAS screen to capture cookies
        await fetchFaupasScreen(appBaseURL, jobBaseURL);
        
        // Step 2: Fetch API token
        const apiToken = await fetchApiToken(jobBaseURL, sessionID, token);
        
        // Step 3: Validate security token
        await validateSecurityToken(jobBaseURL, appBaseURL, sessionID, token, apiToken);
    ...
    async function fetchAssetDetails(appBaseURL, assetID) {
    ...
    async function fetchAttachments(appBaseURL, assetItem) {
      const attachmentResponse = await fetch(`${appBaseURL}Production-UI/api/finance/legacy/documents/FAIdnt/attachments/`, {
        method: "POST",
        headers: {
          accept: "application/json, text/plain, */*",
          "accept-language": "en-US,en;q=0.9",
    ...
    function createDocumentButton(container, documents, assetID) {
    ...
    async function processAssetDocuments(appBaseURL, spans) {
      ...
      const fetchPromises = Array.from(spans).map((span) => {
        ...
        return fetchAssetDetails(appBaseURL, assetID)
          .then(assetItem => fetchAttachments(appBaseURL, assetItem))
          .then(data => {
            // Remove the clock SVG placeholder
            container.innerHTML = "";
            if (data && data.length > 0) {
              console.log("Found", data.length, "documents for asset ID:", assetID);
              createDocumentButton(container, data, assetID);
            } else {
    ...
    // Function to attach definitions
    ...
    // Main execution - extract data
    // Extract sessionID, token, and URLs from span
 try {
    ...
    // 1. Authenticate
    await authenticate(appBaseURL, jobBaseURL, sessionID, token);
    
    // 2. Process asset documents
    const spans = document.querySelectorAll(`span[id^="documentOnline-"]`);
    await processAssetDocuments(appBaseURL, spans);
    
    // 3. Get and process BT models (optional)
    const arrayOfDefs = await getBTModels(appBaseURL, jobBaseURL);
    console.log("BT Models:", arrayOfDefs);
    
    if (arrayOfDefs && arrayOfDefs.length > 0) {
      // Add extra definitions if needed
      const extraDefs = ["BT20.PENameMaster", "BT20.OHZRepeatMaster", "BT20.PEAddrDetail", "BT20.PEVendorDetail"];
      const allDefs = [...arrayOfDefs, ...extraDefs];
